...
export const Table: FC<Props> = ({details} : Props) => {
    // const types = details?.types.map((p: { type: { type: any; }; }) => p.type.type).join(', ');
    const types = details?.types.map((p: { type: { type: any; }; }) => `${p.type.type.charAt(0).toUpperCase() + p.type.type.slice(1).toLowerCase()}`).join(', ');
    const abilities = details?.abilities.map((p: { ability: { ability: any; }; }) => p.ability.ability).join(', ');
    const id = details.id;
