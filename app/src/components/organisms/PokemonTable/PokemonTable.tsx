...
import { useAbilitiesQuery, usePokemonQuery } from 'graphql/generated';
...
export const PokemonTable: FC<Props> = (props) => {
    const { data } = usePokemonQuery();
    const [info, setInfo] = useState<{name: string, japName: string, hp: number, defense: number, attack: number, weight: number, speed: number, types: string}[]>([]);
    ...
    const getPokemonInfo = async () => {
        try{
            ...
            const newInfo = information?.map((info) => ({ 
                ...
                speed: info.speed, 
                types: info.types.filter(p => p.type.type).map(p => p.type.type).join(', '), 
    ...
    return(
        <div>
            <div style={{maxHeight: "400px", overflowY: 'scroll', border: 'solid', textAlign: "center", marginBottom: '25px'}}>
                {/* {info && <AbilityT columns={columns} data={info} />}  */}
                <AbilityT columns={columns} data={info} search={searchClick} />
            </div>
            <div style={{textAlign: 'center', marginTop: '25px'}}>
                <PokemonImage search={search} />
            </div>
        </div>
    )
};